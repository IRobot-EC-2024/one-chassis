	...
	else if(hcan->Instance == CAN1)
	{
		switch(rx_header.StdId)
		{
			...
			case CMSRecceiveID:
			{
				...
				int16_t i = ((uint16_t)rx_data[2] << 8| rx_data[3]);
				int16_t v = ((uint16_t)rx_data[0] << 8| rx_data[1]);
				CMS_Data.chassis_power = int16_to_float(i,32000, -32000,500, 0);
...
void TimerTaskLoop100Hz()
{
	...
	if (cms_send_period == 2)
	{
		...
		cms_send_period=0;
		CMS_POWER_SEND(robot_state.chassis_power_limit,CMS_Data.output_power_limit,CMS_Data.input_power_limit,1);
	}
}
